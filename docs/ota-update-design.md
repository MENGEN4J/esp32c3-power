...
托管在 Gitee 仓库的 `releases/latest/version.json`：
  ...
  "files": [
    {
      ...
      "hash": "a1b2c3d4",
      "size": 5234,
      "url": "https://gitee.com/user/esp32-power/raw/v1.9.0/app.py"
    },
    {
      "name": "wifi_manager.py",
      "hash": "e5f6g7h8",
      "size": 14820,
      "url": "https://gitee.com/user/esp32-power/raw/v1.9.0/wifi_manager.py"
    }
  ]
}
```
...
| `files[].url` | string | Gitee Raw 文件直链 |
...
- Gitee Raw URL 会 302 重定向，需处理重定向（最多 3 次）
...
| `OTA_VERSION_URL` | version.json 的 Gitee Raw 地址 |
...
| Gitee 不可达 | HTTP 超时 10 秒，超时后跳过更新检查 | 正常功能不受影响 |
...
python3 scripts/gen_version_json.py --version 1.9.0 --min-version 1.8.0 \
  ...
  --gitee-user user --gitee-repo esp32-power
```
...
| 问题 | 风险等级 | 说明 |
|------|---------|------|
| 代码明文暴露 | 中 | 任何人访问 Gitee 仓库都能看到完整源码逻辑 |
...
| 实现复杂度 | 低 | 中 | 中高 |
| Gitee 托管 | 直接 Raw 下载 | 直接 Raw 下载 | 直接 Raw 下载 |
  ...
  "files": [
    {
      ...
      "hash": "a1b2c3d4",
      "size": 8200,
      "url": "https://gitee.com/user/esp32-power/raw/v1.9.0/wifi_manager.mpy"
    }
  ]
}
```
...
# 4. 推送 .mpy 文件和 version.json 到 Gitee
```
...
| Gitee Raw 可能有频率限制 | 频繁请求可能被限流 | 每次配网仅检查 1 次 |
...
| Gitee 不可达 | 断网测试 | 10 秒超时，正常功能不受影响 |
